                                ...
                                <div class="p-5 flex-1 flex flex-col">
                                    ...
                                    <div class="pt-4 mt-auto border-t border-gray-100 dark:border-gray-700 flex items-center justify-between">
                                        ...
                                        <div class="flex items-center gap-1">
                                            <button
                                                wire:click="toggleCompleted({{ $todo->id }})"
                                                class="p-2 text-gray-400 hover:text-yellow-600 dark:hover:text-yellow-400 hover:bg-yellow-50 dark:hover:bg-yellow-900/20 rounded-lg transition"
            ...
            @elseif($currentView === 'activity-log')
                @if($activityLogs->count() > 0)
                    <div class="max-w-5xl mx-auto space-y-4">
                        @foreach($activityLogs as $log)
                            <div class="border-l-4 {{ $this->getLogBorderColor($log->action) }} bg-white dark:bg-slate-800 rounded-lg p-5 shadow-sm hover:shadow-md transition-shadow">
                                <div class="flex items-start space-x-4">
                                    <div class="flex-shrink-0 mt-1">
                                        <div class="w-12 h-12 rounded-full {{ $this->getLogBgColor($log->action) }} flex items-center justify-center">
                                            <i class="{{ $this->getLogIcon($log->action) }} text-white text-2xl"></i>
                                        </div>
                                    </div>
                                    <div class="flex-1 min-w-0">
                                        <div class="flex items-start justify-between">
                                            <div class="flex-1">
                                                <div class="flex items-center space-x-2 mb-2">
                                                    <span class="font-bold text-lg text-gray-800 dark:text-gray-100">{{ $this->getLogTitle($log->action) }}</span>
                                                    <span class="text-xs {{ $this->getLogBorderColor($log->action) }} bg-opacity-10 px-2 py-1 rounded font-medium">
                                                        {{ strtoupper($log->action) }}
                                                    </span>
                                                </div>

                                                <p class="text-gray-700 dark:text-gray-300 mb-2">{{ $log->description }}</p>

                                                @if($log->todo)
                                                    <p class="text-sm text-gray-600 dark:text-gray-400 mb-3">
                                                        <span class="font-medium">Catatan:</span>
                                                        <span class="text-blue-600 dark:text-blue-400">{{ $log->todo->title }}</span>
                                                    </p>
                                                @endif

                                                @if($log->action === 'deleted' && $log->metadata)
                                                    <div class="mt-4 p-4 bg-gray-50 dark:bg-slate-700/50 rounded-lg text-sm text-gray-700 dark:text-gray-300 space-y-2">
                                                        <div class="grid grid-cols-2 gap-4">
                                                            <div>
                                                                <div class="flex justify-between">
                                                                    <span class="font-medium text-gray-600 dark:text-gray-400">ID Catatan:</span>
                                                                    <span class="font-mono">{{ $log->metadata['id'] ?? '-' }}</span>
                                                                </div>
                                                                <div class="flex justify-between mt-2">
                                                                    <span class="font-medium text-gray-600 dark:text-gray-400">Dihapus oleh:</span>
                                                                    <span class="font-semibold text-indigo-600 dark:text-indigo-400">{{ $log->metadata['deleted_by']['name'] ?? '-' }}</span>
                                                                </div>
                                                                <div class="flex justify-between mt-2">
                                                                    <span class="font-medium text-gray-600 dark:text-gray-400">Email:</span>
                                                                    <span class="text-blue-600 dark:text-blue-400">{{ $log->metadata['deleted_by']['email'] ?? '-' }}</span>
                                                                </div>
                                                            </div>
                                                            <div>
                                                                <div class="flex justify-between">
                                                                    <span class="font-medium text-gray-600 dark:text-gray-400">Kategori:</span>
                                                                    <span class="font-semibold">{{ ucfirst($log->metadata['category'] ?? '-') }}</span>
                                                                </div>
                                                                <div class="flex justify-between mt-2">
                                                                    <span class="font-medium text-gray-600 dark:text-gray-400">Deadline:</span>
                                                                    <span>{{ $log->metadata['deadline'] ?? '-' }}</span>
                                                                </div>
                                                                <div class="flex justify-between mt-2">
                                                                    <span class="font-medium text-gray-600 dark:text-gray-400">Dibuat:</span>
                                                                    <span>{{ $log->metadata['created_at'] ?? '-' }}</span>
                                                                </div>
                                                            </div>
                                                        </div>

                                                        <div class="pt-3 border-t border-gray-200 dark:border-gray-600 mt-2">
                                                            <span class="font-medium text-gray-600 dark:text-gray-400">Dihapus pada:</span>
                                                            <span class="ml-2 text-red-600 dark:text-red-400 font-semibold">{{ $log->metadata['deleted_at'] ?? '-' }}</span>
                                                        </div>

                                                        @if(isset($log->metadata['description_preview']))
                                                            <div class="pt-3 border-t border-gray-200 dark:border-gray-600">
                                                                <span class="font-medium text-gray-600 dark:text-gray-400">Deskripsi:</span>
                                                                <p class="mt-2 text-gray-700 dark:text-gray-300 bg-white dark:bg-slate-800 p-3 rounded">{{ $log->metadata['description_preview'] }}</p>
                                                            </div>
                                                        @endif
                                                    </div>
                                                @endif

                                                @if($log->action === 'created' && $log->metadata)
                                                    <div class="mt-3 flex flex-wrap gap-2">
                                                        @if(isset($log->metadata['category']))
                                                            <span class="text-xs px-3 py-1.5 bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 rounded-full font-medium">
                                                                Kategori: {{ ucfirst($log->metadata['category']) }}
                                                            </span>
                                                        @endif
                                                        @if(isset($log->metadata['has_media']) && $log->metadata['has_media'])
                                                            <span class="text-xs px-3 py-1.5 bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-300 rounded-full font-medium">
                                                                📷 Ada Foto
                                                            </span>
                                                        @endif
                                                    </div>
                                                ...
                                                @if($log->action === 'updated' && $log->metadata)
                                                    <div class="mt-3 flex flex-wrap gap-2">
                                                        @if(isset($log->metadata['old_category']) && isset($log->metadata['new_category']))
                                                            <span class="text-xs px-3 py-1.5 bg-yellow-100 dark:bg-yellow-900/30 text-yellow-700 dark:text-yellow-300 rounded-full font-medium">
                                                                Kategori: {{ ucfirst($log->metadata['old_category']) }} → {{ ucfirst($log->metadata['new_category']) }}
                                                            </span>
                                                        @endif
                                                        @if(isset($log->metadata['has_media']) && $log->metadata['has_media'])
                                                            <span class="text-xs px-3 py-1.5 bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-300 rounded-full font-medium">
                ...
                @else
                    <div class="text-center max-w-2xl mx-auto py-20">
                        <div class="bg-gray-100 dark:bg-slate-700 w-24 h-24 rounded-full flex items-center justify-center mx-auto mb-6">
                            <i class="ph ph-clock-counter-clockwise text-5xl text-gray-400 dark:text-gray-300"></i>
                        </div>
                        <h3 class="text-2xl font-bold text-gray-700 dark:text-gray-300 mb-3">Belum ada aktivitas</h3>
                        <p class="text-gray-500 dark:text-gray-400 mb-6">Aktivitas seperti menambah, mengedit, atau menghapus catatan akan muncul di sini.</p>
                        <button
                            wire:click="changeView('notes')"
